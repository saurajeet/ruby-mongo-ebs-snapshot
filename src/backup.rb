...
class Backup
	attr_accessor :instance_id, :volume_id, :infra, :device, :settings, :mongo_client
	def initialize
	...
	def backup_vol
		...
		@infra.instances[@instance_id].block_devices.each do |map|
			...
			vol_attr = map[:ebs] 
			puts "Device doesnot exist" & return if vol_attr == nil
			puts "A live Volume should be attached and recording" & return if (map[:ebs][:status] != "attached")
